...
"SELECT projects.category, users.name, SUM(pledges.amount) FROM pledges JOIN users ON pledges.user_ID = users.id JOIN projects ON pledges.project_ID = projects.id WHERE projects.category = 'books'"
end
